...
class RegisterView(generic.FormView):
    ...
    def form_valid(self, form):
        ...
        send_newly_registered_email(user.email, settings.DEFAULT_FROM_EMAIL,
                                    user.get_full_name(), activate_link, settings.IDP_NAME,user.username)
    ...
    def get(self, request, activation_code):
        ...
        if settings.IDP_CATCH_ALL:
           ...
           #                           'Country: ' + user_rr.country]
             ...
             modlist['description'] = [user_rr.organization]
...
class PasswordResetView(generic.FormView):
    ...
    def form_valid(self, form):
        ...
        send_reset_password_email(user.email, settings.DEFAULT_FROM_EMAIL,
                                  user.get_full_name(), reset_link, settings.IDP_NAME,user.username)
...
class DeleteView(generic.View):
    def get(self, request):
        ldap_ops = LDAPOperations()
        ldap_ops.delete_user(request.user.username)
        return redirect("https://ermis-f.eu")
        #return redirect("https://service.ermis-f.eu/logout/")

class ProfileEditView(generic.View):
    template_name = 'dl_user/profile_edit.html'
    ...
    def get(self, request, *args, **kwargs):
        ...
        form = ProfileEditForm(**kwargs)
        context = {'form': form}
        return render(request, 'dl_user/profile_edit.html', context)
    
    def post(self, request, *args, **kwargs):
        #initial = {'complaint': 'I am unhappy with this order!', 'user_name': self.request.user.get_full_name()}i
        ldap_ops = LDAPOperations()
        kwargs = {'user': request.user}
        form = ProfileEditForm(data=request.POST, **kwargs)
        if form.is_valid():
            #self.send_mail(form.cleaned_data)
            data = form.cleaned_data
            ldap_ops.edit_profile(data.get('username'),data.get('first_name'),data.get('last_name'),data.get('organization').name)
            request.user.first_name=data.get('first_name')
            request.user.last_name=data.get('last_name')
            request.user.organization=data.get('organization')
            kwargs = {'user': request.user}
            form = ProfileEditForm(**kwargs)
            context = {'form': form, 'message':_("Your profile has been updated. The changes will take place the next time you will login.")}
            ...
            if request.GET.get('next'):
                #logger.error("1")
                #request.user.message_set.create(message = "hh")
